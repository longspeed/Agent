-- Delete active contact work while retaining the minimum tombstone required
-- to prevent Gmail discovery from silently importing the same thread again.
...
begin
    ...
    -- A send claim is the only durable evidence available when Gmail times
    -- out after accepting a message. Never erase it, even if a delete request
    -- raced past the server's friendly preflight.
    if exists (
    ...
    -- Alerts are optional during the pilot rollout. Older projects may have
    -- the reply desk without the notification-outbox migration, so contact
    -- deletion must not depend on that table. PostgreSQL only prepares the
    -- UPDATE when this branch runs, making the function safe in both schemas.
