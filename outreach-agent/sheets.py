...
def is_verified(row):
    return row[COL_EMAIL_CONFIDENCE].strip().lower() == "verified"
...
def campaign_readiness(account):
    ...
    pending = [(idx, row) for idx, row in rows if not row[COL_STATUS].strip() and row[COL_EMAIL].strip()]
    eligible = [(idx, row) for idx, row in pending if is_verified(row)]
    unverified = [(idx, row) for idx, row in pending if not is_verified(row)] + [
        (idx, row) for idx, row in rows
        if row[COL_STATUS].strip() == "Needs verification" and row[COL_EMAIL].strip()
    ]
